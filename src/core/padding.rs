    ...
    where P: Pixel + Zero
{
...

    {
        ...
        let mut fill_corner = |x, y, val| {
            padded.sub_image_mut(Rect::new(x, y, radius, radius)).fill(val);
        };
        ...
        fill_corner(img.width() + radius, img.height() + radius, img.get_pixel(img.width() - 1, img.height() - 1));
    }
    {
        // Fill the top side
    ...
    }
    {
        ...
        let mut outer_iter = padded.sub_image_mut(Rect::new(img.width() + radius, radius, radius, img.height()));
        for (mut row, value) in outer_iter.rows_mut().zip(inner_iter) {
            row.fill(value.clone());
        }
    }
    {
        ...
        let mut outer_iter = padded.sub_image_mut(Rect::new(radius, img.height() + radius, img.width(), radius));
        for (mut col, value) in outer_iter.cols_mut().zip(inner_iter) {
            col.fill(value.clone());
        }
    }
    ...
    where P: Pixel + Zero
{
...

    {
        ...
        copy_subimage(Rect::new(0, 0, radius, radius),
                      Rect::new(img.width() + radius, img.height() + radius, radius, radius));
        copy_subimage(Rect::new(img.width() - radius, 0, radius, radius),
                      Rect::new(0, img.height() + radius, radius, radius));
        copy_subimage(Rect::new(0, img.height() - radius, radius, radius),
                      Rect::new(img.width() + radius, 0, radius, radius));
        copy_subimage(Rect::new(img.width() - radius, img.height() - radius, radius, radius),
                      Rect::new(0, 0, radius, radius));
        copy_subimage(Rect::new(0, 0, img.width(), radius),
                      Rect::new(radius, img.height() + radius, img.width(), radius));
        copy_subimage(Rect::new(0, 0, radius, img.height()),
                      Rect::new(img.width() + radius, radius, radius, img.height()));
        copy_subimage(Rect::new(img.width() - radius, 0, radius, img.height()),
                      Rect::new(0, radius, radius, img.height()));
        copy_subimage(Rect::new(0, img.height() - radius, img.width(), radius),
    ...
    where P: Pixel + Zero
{
...

    {
        let mut copy_and_mirror_subimage_both = |src_rect, dst_rect| {
            let (src_subimg, mut dst_subimg) = (img.sub_image(src_rect), padded.sub_image_mut(dst_rect));
        ...
        copy_and_mirror_subimage_both(Rect::new(0, 0, radius, radius),
                                      Rect::new(0, 0, radius, radius));
        copy_and_mirror_subimage_both(Rect::new(img.width() - radius, 0, radius, radius),
                                      Rect::new(img.width() + radius, 0, radius, radius));
        copy_and_mirror_subimage_both(Rect::new(0, img.height() - radius, radius, radius),
                                      Rect::new(0, img.height() + radius, radius, radius));
        copy_and_mirror_subimage_both(Rect::new(img.width() - radius, img.height() - radius, radius, radius),
                                      Rect::new(img.width() + radius, img.height() + radius, radius, radius));
    }
    {
        ...
        copy_and_mirror_subimage_hor(Rect::new(0, 0, radius, img.height()),
                                     Rect::new(0, radius, radius, img.height()));
        copy_and_mirror_subimage_hor(Rect::new(img.width() - radius, 0, radius, img.height()),
                                     Rect::new(img.width() + radius, radius, radius, img.height()));
    }
    {
        ...
        copy_and_mirror_subimage_ver(Rect::new(0, 0, img.width(), radius),
                                     Rect::new(radius, 0, img.width(), radius));
        copy_and_mirror_subimage_ver(Rect::new(0, img.height() - radius, img.width(), radius),
                                     Rect::new(radius, img.height() + radius, img.width(), radius));
    }
