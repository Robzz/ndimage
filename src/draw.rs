    ...
    P: Pixel
{
    ...
    if coords.0 < w && coords.1 < h {
        ...
        for pix in img.row_mut(coords.1).unwrap().skip(xs as usize).take((xe - xs + 1) as usize) {
            *pix = color.clone();
        }
        for pix in img.col_mut(coords.0).unwrap().skip(ys as usize).take((ye - ys + 1) as usize) {
            *pix = color.clone();
        }
    }
}
